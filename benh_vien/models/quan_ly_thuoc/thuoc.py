...
class Thuoc(models.Model):
    ...
    ma_thuoc = fields.Char(string="Mã thuốc", required=True, copy=False, readonly=True,default="New")
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if 'ma_thuoc' not in vals or vals['ma_thuoc'] == 'New':
                # Lấy số tiếp theo từ sequence 'benhvien.thuoc' để tạo mã thuốc
                vals['ma_thuoc'] = self.env['ir.sequence'].next_by_code('benhvien.thuoc') or 'New'
    ...
    @api.depends("lohang_ids.so_luong_ton_kho")
    def _compute_so_luong_ton_kho(self):
        """Tính tổng số lượng tồn kho dựa trên số lượng tồn của các lô hàng."""
        for record in self:
            record.so_luong_ton_kho = sum(record.lohang_ids.mapped("so_luong_ton_kho"))